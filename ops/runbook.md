...
- **Phase 10 (acceptance):** ran `verify-node.sh` for real. 13/14 executable tests passed (test 15, the control-plane reachability check, correctly SKIPPED rather than faking a pass — no public control-plane URL yet). Fixed two real bugs found while running it: test 5 previously always passed regardless of outcome, and cleanup used a nonexistent `droprole` command (correct is `dropuser`) in three places. `ynai` was verified byte-identical (`pg_database_size`, table count) before and after the full run.
